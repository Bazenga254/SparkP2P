  ...
  for (const order of orders.sell) {
    ...
    // ── Awaiting buyer payment ──────────────────────────────────────────────
      ...
      delete orderLastBotReplyAt[order.orderNumber];
        ...
        // ── Send ONE payment reminder — generated by AI, never repeated ────────
        console.log(`[SparkP2P] Order ${order.orderNumber} — no payment after ${seenMins}m, sending reminder`);
        const chatCtxR = await analyzeChatHistory(page);
        const reminderMsg = await generateUniqueMessage('awaiting_payment_reminder', chatCtxR, { amount: order.totalPrice })
          || `Hi! Just a friendly reminder that your order of KES ${order.totalPrice} is still awaiting payment. Please complete your M-Pesa transfer and share the confirmation code in the chat once done. Thank you!`;
        await sendChatMessage(page, reminderMsg);
        orderReminderSent.add(order.orderNumber);
        orderLastBotReplyAt[order.orderNumber] = Date.now();

      } else if (orderReminderSent.has(order.orderNumber)) {
        // ── Reminder already sent — check if buyer has asked a question ─────────
        const chatCtxQ = await analyzeChatHistory(page);
        const lastReply = orderLastBotReplyAt[order.orderNumber] || 0;
        const minsSinceReply = (Date.now() - lastReply) / 60000;

        if (chatCtxQ && chatCtxQ.last_sender === 'buyer' && minsSinceReply >= 2) {
          // Buyer sent a message we haven't responded to yet — reply with AI
          console.log(`[SparkP2P] Order ${order.orderNumber} — buyer sent a message, generating AI response`);
          const aiReply = await generateUniqueMessage('buyer_question', chatCtxQ, {
            amount: order.totalPrice,
            buyerMessage: chatCtxQ.last_buyer_message || chatCtxQ.conversation_summary,
          });
          if (aiReply) {
            await sendChatMessage(page, aiReply);
            orderLastBotReplyAt[order.orderNumber] = Date.now();
            console.log(`[SparkP2P] ✅ Replied to buyer question`);
          }
        } else {
          console.log(`[SparkP2P] Order ${order.orderNumber} — awaiting payment (${seenMins}m, ${countdown !== null ? countdown + 's left' : 'no timer'}) — silent wait`);
        }
      }
