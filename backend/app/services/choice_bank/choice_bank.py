...
async def choice_bank_webhook(request: Request):
    ...
    if notification_type == "0002":
        ...
        await _handle_balance_change(params)
    else:
...
async def _handle_transaction_result(params: dict, raw: dict):
    ...
    logger.info(
        f"[ChoiceBank] 0002: txId={tx_id}, account={account_id}, "
        f"status={tx_status}, type={tx_type}, KES {amount}"
    )

    # Only process successful inbound (credit) transactions
    if tx_status.upper() not in ("SUCCESS", "COMPLETED", "00000", "S"):
        logger.info(f"[ChoiceBank] Skipping non-success transaction: status={tx_status}")
        return

    # txType for an inbound credit typically contains "IN", "CREDIT", "C2B", or similar.
    # We skip obvious outbound debits to avoid double-processing.
    if tx_type and any(k in tx_type.upper() for k in ("OUT", "DEBIT", "B2C", "WITHDRAW")):
    ...
    async with async_session() as db:
        ...
        # Amount tolerance ±KES 5
        if abs(order.fiat_amount - amount) > 5:
            logger.warning(
                f"[ChoiceBank] Amount mismatch for order {order.binance_order_number}: "
        ...
        # Mark order as payment received — bot picks this up and releases crypto
        ...
        db.add(WalletTransaction(
            ...
            description="Choice Bank inbound payment",
        ))

        await db.commit()
        logger.info(
            f"[ChoiceBank] MATCHED: {tx_id} → order {order.binance_order_number} "
            f"(Trader: {trader.full_name}, KES {amount}) → PAYMENT_RECEIVED"
        )
...
async def confirm_onboarding_otp(body: OtpConfirmRequest, db: AsyncSession = Depends(get_db)):
    """Confirm the OTP received by the trader to proceed with account creation."""
