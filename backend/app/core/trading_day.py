...
def trading_week_start(now=None):
    """UTC datetime marking the start of the current trading WEEK (Monday 00:00).

    Calendar-aligned, like trading_month_start — NOT a rolling 7-day window. This
    keeps 'This week' consistent with 'Today' and 'This month' (all "so far this
    period"), so on the 5th of a month 'this week' can't reach back into last month
    and outrun 'this month'.
    """
    now = _as_utc(now) if now is not None else now_utc()
    off = timedelta(hours=TRADING_DAY_OFFSET_HOURS)
    shifted = now + off
    monday = shifted.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=shifted.weekday())
    return monday - off
